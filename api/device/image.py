...
class Image:
    ...
    def screenshot(self, rect: List[int] = None) -> Union[
        None, bytes]:
        """截取设备屏幕"""
        ret = self._api._call_and_parse(CommonResponse, self._api._payload.pic_screenshot, 
                                   self._device_id, False, rect)
        if isinstance(ret, bytes):
            return ret
        self._api.successful(ret)
        return None
    ...
    def find_image(self, img_list: List[str], similarity: float = 0.85, all: bool = False,
        ...
        """普通找图"""
        ret = self._api._call_and_parse(FindImageResultResponse, self._api._payload.pic_find_image,
    ...
    def find_image_cv(self, img_list: List[str], similarity: float = 0.85, all: bool = False,
        ...
        """OpenCV找图"""
        ret = self._api._call_and_parse(FindImageCvResultResponse, self._api._payload.pic_find_image_cv,
    ...
    def ocr(self, rect: List[int] = None, is_ex: bool = False) -> List[OcrResult]:
        """OCR文字识别"""
        ret = self._api._call_and_parse(OcrResultResponse, self._api._payload.pic_ocr,
    ...
    def find_text(self, text_list: List[str], similarity: float, contain: bool = False,
        ...
        """查找文字"""
        ret = self._api._call_and_parse(OcrResultResponse, self._api._payload.pic_find_text,
    ...
    def find_multi_color(self, params: List[MultiColorParams], all: bool = False,
        ...
        """多点找色"""
        ret = self._api._call_and_parse(FindMultiColorResponse, self._api._payload.pic_find_multi_color,
