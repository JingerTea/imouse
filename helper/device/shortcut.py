...
class Shortcut(BaseAPI):
    ...
    def album_get(self, album_name: str = "", num: int = 20,
        ...
        """获取相册列表"""
        ret = self._call_and_parse(AlbumFileResponse, self._payload.shortcut_album_get,
    ...
    def album_update(self, files: List[str], album_name: str = "", is_zip: bool = False,
        ...
        """上传文件到相册"""
        ret = self._call_and_parse(AlbumFileResponse, self._payload.shortcut_album_upload,
    ...
    def album_down(self, files: List[AlbumFileParams], is_zip: bool = False,
                   outtime: int = 30) -> bool:
        """下载相册文件"""
        ret = self._call_and_parse(CommonResponse, self._payload.shortcut_album_down,
    ...
    def album_del(self, files: List[AlbumFileParams], outtime: int = 30) -> List[AlbumFileResult]:
        """删除相册文件"""
        ret = self._call_and_parse(AlbumFileResponse, self._payload.shortcut_album_del,
    ...
    def album_clear(self, album_name: str = '', outtime: int = 30) -> List[AlbumFileResult]:
        """清空相册文件"""
        ret = self._call_and_parse(AlbumFileResponse, self._payload.shortcut_album_clear,
    ...
    def file_get(self, path: str = "",
        ...
        """获取文件列表"""
        ret = self._call_and_parse(PhoneFileResponse, self._payload.shortcut_file_get,
    ...
    def file_upload(self, files: List[str], path: str, is_zip: bool = False,
        ...
        """上传文件到手机"""
        ret = self._call_and_parse(PhoneFileResponse, self._payload.shortcut_file_upload,
    ...
    def file_down(self, path: str, files: List[PhoneFileParams], is_zip: bool = False,
                  outtime: int = 30) -> bool:
        """从手机下载文件"""
        ret = self._call_and_parse(CommonResponse, self._payload.shortcut_file_down,
                                       self._device_id, path, is_zip, files, outtime)
        return self._device.successful(ret)
    ...
    def file_del(self, path: str, files: List[PhoneFileParams],
                 outtime: int = 30) -> List[FileResult]:
        """删除手机文件"""
        ret = self._call_and_parse(PhoneFileResponse, self._payload.shortcut_file_del,
    ...
    def clipboard_set(self, text: str, sleep: int = 0, outtime: int = 10) -> bool:
        """发送文字到手机剪切板"""
        ret = self._call_and_parse(CommonResponse, self._payload.shortcut_clipboard_set,
                                       self._device_id, text, sleep, outtime)
        return self._device.successful(ret)

    def clipboard_get(self, outtime: int = 10) -> str:
        """获取手机剪切板内容"""
        ret = self._call_and_parse(ResultTextResponse, self._payload.shortcut_clipboard_get,
    ...
    def exec_url(self, url: str, outtime: int = 10) -> bool:
        """打开url"""
        ret = self._call_and_parse(CommonResponse, self._payload.shortcut_exec_url,
                                       self._device_id, url, outtime)
        return self._device.successful(ret)

    def switch_device(self, state: int, outtime: int = 10) -> bool:
        """重启关闭手机"""
        ret = self._call_and_parse(CommonResponse, self._payload.shortcut_switch_device,
                                       self._device_id, state, outtime)
        return self._device.successful(ret)

    def switch_bril(self, state: float, outtime: int = 10) -> bool:
        """设置屏幕亮度"""
        ret = self._call_and_parse(CommonResponse, self._payload.shortcut_switch_bril,
                                       self._device_id, state, outtime)
        return self._device.successful(ret)

    def switch_torch(self, state: int, outtime: int = 10) -> bool:
        """开关手电筒"""
        ret = self._call_and_parse(CommonResponse, self._payload.shortcut_switch_torch,
                                       self._device_id, state, outtime)
        return self._device.successful(ret)

    def switch_flight(self, state: int, outtime: int = 10) -> bool:
        """开关飞行模式"""
        ret = self._call_and_parse(CommonResponse, self._payload.shortcut_switch_flight,
                                       self._device_id, state, outtime)
        return self._device.successful(ret)

    def switch_cdpd(self, state: int, outtime: int = 10) -> bool:
        """开关蜂窝数据"""
        ret = self._call_and_parse(CommonResponse, self._payload.shortcut_switch_cdpd,
                                       self._device_id, state, outtime)
        return self._device.successful(ret)

    def switch_wlan(self, state: int, outtime: int = 10) -> bool:
        """开关无线局域网"""
        ret = self._call_and_parse(CommonResponse, self._payload.shortcut_switch_wlan,
                                       self._device_id, state, outtime)
        return self._device.successful(ret)

    def device_ip(self, outtime: int = 10) -> str:
        """获取外网ip"""
        ret = self._call_and_parse(ResultTextResponse, self._payload.shortcut_device_ip,
